"""SDD missing-history v1 双向 Mamba 臂：训练 + held-out test（单 condition）。

与 run_sdd_missing.py 完全同协议，仅两处差异：
1. 配置 config_sdd_missing_bimamba（model bimamba: true）
2. 输出/summary 文件名带 _bimamba 后缀，避免污染单向结果

用法:
    CUDA_VISIBLE_DEVICES=<gpu> python scripts/训练与评估/run_sdd_missing_bimamba.py <condition> [epochs]
"""
import csv
import os
import re
