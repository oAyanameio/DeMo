#!/bin/bash
# M2_history 独立五折（GPU3）——提速并行臂；完成条件 folds_ok=5。
set -uo pipefail
cd /home/lbh/DeMo
...
CUDA_VISIBLE_DEVICES=3 PYTHONPATH=. "$PY" scripts/训练与评估/run_missing_aware_ethucy.py \
  ...
  --num-workers 16 \
  --precision bf16 \
  > "$LOG/M2_history_random_fixed4_ng_seed2024.out" 2>&1
rc=$?
...
echo "[$(date '+%F %T')] M2_history rc=$rc folds_ok=$ok" >> "$LOG/chain_random_fixed4_ng_seed2024.log"
