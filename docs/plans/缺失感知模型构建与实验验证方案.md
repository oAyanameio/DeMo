...
> **方法层改版同步（2026-09-08）**：M1–M4 方法层已按"证据中心预测"框架重构为 M1'/M2'/M3'/M4'（证据时钟解码 / 社会证据补偿 / 证据校准 / 缺失增广训练），权威定义见《[选题](../research/选题.md)》§1.2 误差结构分解与 §五.0A 方法框架（2026-09-08 已与原《选题（改）》合并为唯一选题文档）。本方案 §2.3/§2.4/§2.5 中的旧 `M1_obs`–`M4_full` 开关矩阵、§5 的 v1/v2/v3 筛选矩阵属于**已实现并完成首筛的历史工程路线**，保留作为输入特征工程路线的对照证据与代码资产；新的 M1'–M4' 实现任务、验收标准与实验矩阵将另行增补（增补前不得启动新方法层正式实验）。数据协议（Clean-direct=完整 ETH/UCY 五折、Easy/Hard-direct=TrajImpute release）与全部验收门槛不变。
>
> **状态（2026-09-08 修正）：benchmark 语义已区分。** `Clean-direct` 使用仓库已审计的原始完整 ETH/UCY 五折数据 `data/ETHUCY_benchmark_v1`；`Easy-direct/Hard-direct` 使用 TrajImpute 官方缺失 release pkl。三者均为直接缺失预测路径；官方 `Clean/Easy-impute/Hard-impute` 为**插补后预测**参考协议，仅作有限范围外部对照。本文档中历史 v1/v2/v3 数据协议、条件矩阵与实验设计降级为“历史预实验协议与归档说明”（见 §1.5）。完整 ETH/UCY B1 运动特征和 Clean-direct runner 已接通。**K 口径裁定（2026-09-08）：主链 K=20**（num_modes=20，与 TrajImpute 官方预测协议对齐；历史 K=6 口径不再使用）——本文档 §4/§5 旧任务清单中的 num_modes=6、val_minFDE6 等配置为历史工程记录，重启正式实验前须按 K=20 更新配置与 monitor 指标名。
>
...
- 正式主实验：尚未开始；Clean-direct 已固定为完整 ETH/UCY benchmark，P0/P0.5 可从该数据源启动
...
- SDD 的单种子训练适配结果中，单向模型在 `random_fixed3` 与 `random_block6` 出现早期选点和明显退化；这可能是结构差异，也可能是优化不稳，不能直接写成方法结论。[缺失历史实验总汇总](../results/historical/缺失历史实验总汇总.md)
...
Clean-direct 数据源：`data/ETHUCY_benchmark_v1`，五折 train/val/test 完整历史，manifest `version=ethucy_benchmark_v1` 且 `context_policy=strict_complete`
...
在 Clean-direct 训练数据源被明确写入实验 manifest 前，不得把 Clean-direct 训练结果称为官方 clean 结果，也不得启动 P0/P0.5 的正式可报告实验。
...
- 保留全部数据、代码、审计与结果，用于复现实验历史、说明研究路线演化与定位问题；
- 不再作为论文正式主实验协议，其结果不构成 TrajImpute 官方结果，不与 Clean-direct/Easy-direct/Hard-direct 主结果混合，不用于推导新 benchmark 的提升幅度；
...
- 不重新制作或覆盖 `data/ETHUCY_missing_v1/`、`data/ETHUCY_missing_v2_high/`、`data/SDD_missing_v1/`、`data/SDD_missing_v2_high/`（历史预实验数据，见 §1.5）。
...
> **状态（2026-09-07）：本节全部内容（§5.1–§5.5）基于历史 v1/v2/v3 自定义协议，已归档为历史预实验参考（§1.5），在官方 release 审计与 TrajImpute 适配器完成前不执行。** 正式实验设计以本文 direct 扩展协议（Clean-direct/Easy-direct/Hard-direct）为准，见《选题》§八.0 与 §八.3（P0–P5）；TrajImpute 官方 Clean/Easy-impute/Hard-impute 仅作外部参考。本节保留原文不改写，仅作为历史执行记录。
...
6. 在完整 ETH/UCY 的 Clean-direct 上复现 `M0-current` 与 `B0/B1/B2` 筛选，确认 `M0-strong`。
7. 在 Easy-direct 上复验 `M1-strong`/`M2-strong`，按确认性门槛决定是否进入 `M3/M4`。
...
- [三轮缺失历史实验统一方案](../research/缺失历史实验统一方案.md)
- [第二轮高缺失历史实验方案详版](../research/archive/ETHUCY与SDD第二轮高缺失历史实验方案.md)
- [缺失历史数据集 v1 审计](../audits/缺失历史数据集v1审计.md)
- [缺失历史数据集 v2 高缺失审计](../audits/缺失历史数据集v2高缺失审计.md)
- [缺失历史实验总汇总](../results/historical/缺失历史实验总汇总.md)
