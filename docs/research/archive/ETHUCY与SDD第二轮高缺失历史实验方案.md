...
>
> **归档状态（2026-09-07）：本文档为历史自定义协议 / 归档参考。** 文档保留用于复现实验历史，不再定义论文的正式 benchmark（数据基准来源已改为 TrajImpute，见《选题》§八.0）；本文的实验协议、条件矩阵与相关结果不构成 TrajImpute 官方结果，不与 Clean-direct/Easy-direct/Hard-direct 主结果混合，不用于推导新 benchmark 的提升幅度，仅用于说明研究路线演化、工程问题和历史探索结论。文中技术细节与当时的审计结论保持原样，不修改。
