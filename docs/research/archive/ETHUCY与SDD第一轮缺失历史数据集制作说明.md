...
> **状态（2026-09-07）：本文档为历史自定义协议 / 归档参考。** 文档保留用于复现实验历史，不再定义论文的正式 benchmark（数据基准来源已改为 TrajImpute，见《选题》§八.0）。相关结果不构成 TrajImpute 官方结果，不与 Clean-direct/Easy-direct/Hard-direct 主结果混合，不用于推导新 benchmark 的提升幅度，仅用于说明研究路线演化、工程问题和历史探索结论。文中技术细节与当时的审计结论保持原样，不修改。
...
该约束是第一轮工程兼容条件，不是对真实缺失分布的假设。第二轮高缺失数据仍沿用该约束；若改为“最后有效帧锚定”，再单独制作允许第 6、7 帧缺失的后续协议。

注意：本节只定义已完成的 v1 条件。`random_fixed3`、`random_fixed4`、`random_block3`、`random_block4` 和 `random_block6` 已按第二轮方案生成，并通过全量审计；第二轮构建、审计和读取支持已经落地，训练与零样本实验已于 2026-09-01 启动。`random_fixed5` 仍未生成，属于可选条件。
