...
> **归档状态（2026-09-07）：本文档为历史自定义协议 / 归档参考。** v3 无保护协议及其全部实验结果（含 `M0_base`/`M1_obs`/`M2_history` 首筛）保留用于复现实验历史与问题定位，不再定义论文的正式 benchmark（数据基准来源已改为 TrajImpute，见《选题》§八.0）。统一说明：这些结果来自历史自定义 v3 无保护、train_adapt、单种子探索性实验；其价值是定位问题和指导设计，不构成 TrajImpute 官方结果，也不构成本文 TrajImpute-derived 直接预测主结论；不与 Clean-direct/Easy-direct/Hard-direct 主结果混合，不用于推导新 benchmark 的提升幅度，仅用于说明研究路线演化、工程问题和历史探索结论。文中技术细节与当时的审计结论保持原样，不修改。
